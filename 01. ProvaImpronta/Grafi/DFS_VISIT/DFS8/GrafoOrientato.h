...
template<class T> void GrafoOrientato<T>::DFS_VISIT(Vertice<T>* u)
{
    ...
    /*il contenuto del file di output sarà una lista di valori,
      uno per riga, che indicano i vertici visitati in ordine di visita.*/
    while (!coda_locale.empty())
    {
        fileOut <<"Vertice: " << coda_locale.front() <<endl;
        coda_locale.pop();
    }
    fileOut.close();
}
